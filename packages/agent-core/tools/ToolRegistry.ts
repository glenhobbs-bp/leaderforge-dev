...
export async function getAvailableTools(ctx: ToolContext): Promise<AgentTool[]> {
  return (
    await Promise.all(
      allTools.map(async tool => {
        if (tool.featureFlag) {
          const enabled = await isFeatureEnabled(tool.featureFlag, ctx.userId, ctx.contextKey)

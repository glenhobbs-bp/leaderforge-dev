...
```sql
-- entitlements.sql
-- Comprehensive entitlements and licensing system
...
CREATE INDEX idx_entitlements_module ON core.entitlements(module_id);
...
CREATE INDEX idx_user_entitlements_active ON core.user_entitlements(user_id, entitlement_id)
...
```sql
-- content-tables.sql
-- Module content and user progress tracking
...
CREATE TABLE modules.content (
  ...
  -- Module assignment and categorization
  available_modules TEXT[] NOT NULL,
  categories JSONB DEFAULT '{}', -- {"movement": ["leadership"], "wealth": ["stewardship"]}
  tags TEXT[],
  
  -- Instructor and source info
  instructor_name TEXT,
  instructor_id UUID,
  source_platform TEXT, -- 'tribe', 'internal', 'external'
  ...
  visibility TEXT DEFAULT 'public' CHECK (visibility IN ('public', 'module', 'organization', 'private')),
...
-- Indexes for content
CREATE INDEX idx_content_modules ON modules.content USING GIN(available_modules);
CREATE INDEX idx_content_search ON modules.content USING GIN(search_vector);
CREATE INDEX idx_content_embedding ON modules.content USING ivfflat(embedding vector_cosine_ops);
CREATE INDEX idx_content_published ON modules.content(published_at) WHERE published_at IS NOT NULL;
CREATE INDEX idx_content_type ON modules.content(content_type);
CREATE INDEX idx_content_instructor ON modules.content(instructor_name);
CREATE INDEX idx_content_parent ON modules.content(parent_content_id);
...
CREATE INDEX idx_user_progress_user ON modules.user_progress(user_id);
CREATE INDEX idx_user_progress_content ON modules.user_progress(content_id);
CREATE INDEX idx_user_progress_module ON modules.user_progress(module_context);
CREATE INDEX idx_user_progress_completed ON modules.user_progress(completed_at) WHERE completed_at IS NOT NULL;
CREATE INDEX idx_user_progress_sync ON modules.user_progress(sync_status) WHERE sync_status != 'synced';
CREATE INDEX idx_user_progress_active ON modules.user_progress(user_id, last_viewed_at) WHERE completed_at IS NULL;
```
...
CREATE INDEX idx_conversation_events_type ON core.conversation_events(event_type);
CREATE INDEX idx_conversation_events_module ON core.conversation_events(module_context);
...
CREATE INDEX idx_conversations_module ON core.conversations(module_context);
CREATE INDEX idx_conversations_updated ON core.conversations(updated_at);
...
-- Partition learning events by month for performance
-- (Partitioning setup would be done separately)

-- Indexes for learning events
CREATE INDEX idx_learning_events_user ON analytics.learning_events(user_id);
CREATE INDEX idx_learning_events_type ON analytics.learning_events(event_type);
CREATE INDEX idx_learning_events_module ON analytics.learning_events(module_context);
CREATE INDEX idx_learning_events_created ON analytics.learning_events(created_at);
CREATE INDEX idx_learning_events_content ON analytics.learning_events(content_id);
```
...
INSERT INTO core.entitlements (name, display_name, description, module_id, features, price_monthly, price_annual) VALUES
...
INSERT INTO modules.content (title, description, content_type, available_modules, categories, instructor_name, duration_seconds, published_at) VALUES
...
-- Update search vector on content changes

...
SELECT name, display_name, description, context_key, features::jsonb FROM (
  VALUES
    ...
    ('leaderforge-admin', 'LeaderForge Admin', 'Full administrative access to all LeaderForge business features and content', 'leaderforge', '{"allContent": true, "allFeatures": true, "business": true, "businessCoaching": true, "training": true, "teamDashboard": true, "boldActions": true, "analytics": true, "adminPanel": true, "userManagement": true, "executiveSessions": true}'),
...
-- Now grant ALL of these entitlements to Glen's user account (only if not already granted)
